...
Google スプレッドシート向けのデータバリデーションカスタム関数。TypeScript で実装し、Google Apps Script として動作する。

このリポジトリは [apps-script-fleet](https://github.com/h13/apps-script-fleet) テンプレートから生成されました。
...
3. `.clasp-dev.json` と `.clasp-prod.json` に `scriptId` を設定:
   ...
   ```
4. CI/CD シークレット `CLASPRC_JSON` を GitHub リポジトリ または Org レベルに設定
...
```
src/
├── index.ts        # GAS エントリポイント（IS_VALID_EMAIL 等）
└── validators.ts   # バリデーションロジック（純粋関数）
test/
└── validators.test.ts  # ユニットテスト（カバレッジ 100%）
```

- `src/index.ts`: GAS カスタム関数を定義。`export` キーワードなし（GAS の制約）
